...
class FaceCrop():
    ...
    def __init__(self, height, width, height_asy, width_asy, tag='A', pyqt_ui=None):
        ...
        self.modelFile = "./crop/main/res10_300x300_ssd_iter_140000.caffemodel"
        self.configFile = "./crop/main/deploy.prototxt.txt"
        self.net = cv2.dnn.readNetFromCaffe(self.configFile, self.modelFile)


    #finds the face and saves the cropped face in your output directory
    def crop_save(self, input_file, input_folder, output_path, bool_folder=False, bool_face_count=False, preview=False):
        ...
        folder_name = pathlib.PurePath(input_folder).name
        ...
        image = cv2.imdecode(np.fromfile(input_file, dtype=np.uint8), cv2.IMREAD_COLOR)
        ...
        temp_file = np.asarray(ImageOps.exif_transpose(Image.open(input_file)))
